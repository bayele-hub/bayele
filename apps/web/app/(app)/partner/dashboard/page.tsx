...
import { Coins, Handshake, Users, KeyRound, ArrowRight, CheckCircle2, Megaphone } from 'lucide-react';
...
export default async function ConsultantDashboard() {
  ...
  const [{ data: retainers }, { data: profile }, { data: links }, { data: deals }] = await Promise.all([
    supabase
  ...
  const represented = (links ?? []).filter((l) => l.my_side === 'partner' && l.status === 'active').length;
  const commissionsEarned = (deals ?? []).filter((d) => d.commission_status !== 'estimated').reduce((s, d) => s + d.commission_fcfa, 0);
      ...
      <div className="rounded-2xl border border-line bg-gradient-to-br from-brand-50 via-white to-accent-soft p-4 shadow-card">
        ...
        <p className="mt-1 text-xs text-muted">
          {pendingCount > 0
            ...
            : 'Représentez vos créateurs, postulez aux campagnes pour eux et suivez vos commissions.'}
        </p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Stat icon={Users} label="Créateurs représentés" value={String(represented)} tone="brand" />
        <Stat icon={Coins} label="Commissions gagnées" value={fmtFcfa(commissionsEarned)} tone="accent" />
        <Stat icon={Coins} label="Honoraires rétainers" value={fmtFcfa(earned)} tone="accent" />
        <Stat icon={Handshake} label="Contrats actifs" value={String(activeCount)} tone="brand" />
      </div>

      {/* Quick actions */}
      <div className="grid grid-cols-2 gap-3">
        <Link href="/partner/creators" className="group flex items-center gap-3 rounded-2xl border border-line bg-white p-4 shadow-card transition hover:border-brand-100">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-brand-50 text-brand"><Users className="h-5 w-5" /></span>
          <div className="min-w-0"><p className="font-bold text-ink">Mon portefeuille</p><p className="truncate text-[11px] text-muted">Créateurs & commissions</p></div>
        </Link>
        <Link href="/partner/campaigns" className="group flex items-center gap-3 rounded-2xl border border-line bg-white p-4 shadow-card transition hover:border-brand-100">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-accent-soft text-accent"><Megaphone className="h-5 w-5" /></span>
          <div className="min-w-0"><p className="font-bold text-ink">Postuler pour eux</p><p className="truncate text-[11px] text-muted">Campagnes ouvertes</p></div>
        </Link>
