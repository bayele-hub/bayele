-- ============================================================================
-- Rollback of:        20260817120600_0007_rls_initplan_optimization.sql
-- Author:             Principal Backend & Database Systems Engineer
-- Inverse type:       TRUE INVERSE — restore the pre-0007 policy form (auth.uid() unwrapped,
--                     still private.is_admin from 0005). Reintroduces the auth_rls_initplan advisor.
-- ============================================================================
...
CREATE POLICY "campaign_creators visible to creator, owner, admin" ON public.campaign_creators FOR SELECT
  USING (creator_id = auth.uid() OR private.is_admin(auth.uid()) OR EXISTS (SELECT 1 FROM public.campaigns c WHERE c.id = campaign_id AND c.owner_id = auth.uid()));
...
CREATE POLICY "escrow visible to involved parties" ON public.escrow_transactions FOR SELECT
  USING (recipient_profile_id = auth.uid() OR private.is_admin(auth.uid()) OR EXISTS (SELECT 1 FROM public.campaigns c WHERE c.id = campaign_id AND c.owner_id = auth.uid()));
...
CREATE POLICY "proof visible to creator, owner, admin" ON public.proof_of_post FOR SELECT
  USING (private.is_admin(auth.uid()) OR EXISTS (SELECT 1 FROM public.campaign_creators cc JOIN public.campaigns c ON c.id = cc.campaign_id WHERE cc.id = proof_of_post.campaign_creator_id AND (cc.creator_id = auth.uid() OR c.owner_id = auth.uid())));
...
CREATE POLICY "audit visible to involved parties and admin" ON public.escrow_audit_log FOR SELECT
  USING (private.is_admin(auth.uid()) OR EXISTS (SELECT 1 FROM public.escrow_transactions et LEFT JOIN public.campaigns c ON c.id = et.campaign_id WHERE et.id = escrow_audit_log.transaction_id AND (et.recipient_profile_id = auth.uid() OR c.owner_id = auth.uid())));
...
DELETE FROM supabase_migrations.schema_migrations WHERE version = '20260817120600';
